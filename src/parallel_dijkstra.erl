...
main() ->
	...
	PerCore = round_up_div(length(NodesList), NumOfCores), % Calculate the number of nodes per core.
	ListOfProcs = create_procs(Source, Target, Graph, NodesList, PerCore, []). % Create list of processes and store the pids in the lists.

server(ListOfProcs, TempListOfProcs, ClosestVertexes) -> 	
	receive
		{From, {closest, U}} ->
			NewListOfProcs = lists:remove(From, ListOfProcs),
			case NewListOfProcs of 
				[] ->	
					%U = get_constant_node(ClosestVertexes), % TODO: Implement this function.
					case U =:= infinity of
						true ->
							io:format("DeadEnd, could not reach Target from Source");
						false ->
							%send_constant_node_to_all(ListOfProcs, ), % TODO: Implement this function.
							server(ListOfProcs, ListOfProcs, [])
					end;
				_ ->	
					server(ListOfProcs, NewListOfProcs, [U|ClosestVertexes])
			end;
		{From, _} ->
...
loop(Source, Target, Graph, Dist, Previous) ->
	...
	io:format("The process ~p got ~n Dist:~p,~nPrevious:~p~n~n", [self(), Dist, Previous]),  
	exit(self(), debugging),
